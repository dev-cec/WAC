...
\toprule
\textbf{Output, in \fichier{}} & \textbf{Role} \\
\midrule
\endhead
...
\fichier{tests/propsys.dll} & stand-in for \texttt{wine}, which lacks this library \\
\bottomrule
\end{longtable}

\begin{attention}
The stand-in is deliberately in \fichier{build-windows/tests/}, away from
\fichier{WAC.exe}. Beside it, it would be loaded instead of the real library
— Windows looks in the program's folder first. Never copy the
\fichier{tests/} folder onto a collection medium.
\end{attention}
