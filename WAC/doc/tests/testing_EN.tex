...
and on a real collection; they are orders of magnitude.}
\vspace*{2cm}
\clearpage
...
\toprule
\textbf{Output} & \textbf{Role} \\
\midrule
\endhead
\fichier{build-windows/WAC.exe} & the collection tool \\
\fichier{build-windows/raw\_hive\_test.exe} & raw NTFS reader, used by the VM cycle \\
\fichier{build-windows/tests/lnk\_test.exe} & robustness of the shortcut parser \\
\fichier{build-windows/tests/parsers\_test.exe} & robustness of the jump list and Prefetch parsers \\
\fichier{build-windows/tests/propsys.dll}, \fichier{offreg.dll} & stand-ins for \texttt{wine}, which lacks both libraries \\
\bottomrule
\end{longtable}

\begin{attention}
The two stand-ins are deliberately in \fichier{build-windows/tests/}, away from
\fichier{WAC.exe}. Beside it, they would be loaded instead of the real
libraries — Windows looks in the program's folder first — and \wac{} would
read no hive at all, with no visible error. Never copy the \fichier{tests/}
folder onto a collection medium.
\end{attention}
...
\texttt{X} / \texttt{XUtc} pairs & double time-zone shift \\
...
Reads an \fichier{.evtx} outside Windows, under \texttt{wine}. It serves to try
...
The build command is in the README, section \emph{Testing the EVTX parser outside
Windows}.
...
\begin{note}
\texttt{WINEDLLOVERRIDES="propsys=n"} loads the \fichier{propsys.dll} stand-in:
\texttt{wine} does not implement the function that names properties, and would
stop the test at the first real shortcut carrying some.
\end{note}
...
\begin{attention}
A real collection is processed locally. If a hive must be read with the real
\fichier{offreg.dll}, it may be copied into the test VM, then deleted as soon as
the test is over. No copy stays in the temporary working folder, and none of it
