...
\toprule
\textbf{Produit} & \textbf{Rôle} \\
\midrule
\endhead
\fichier{build-windows/WAC.exe} & l'outil de collecte \\
\fichier{build-windows/raw\_hive\_test.exe} & lecteur NTFS brut, utilisé par le cycle VM \\
\fichier{build-windows/tests/lnk\_test.exe} & robustesse de l'analyseur de raccourcis \\
\fichier{build-windows/tests/parsers\_test.exe} & robustesse des listes de sauts et des Prefetch \\
\fichier{build-windows/tests/propsys.dll}, \fichier{offreg.dll} & substituts pour \texttt{wine}, qui n'a pas ces deux bibliothèques \\
\bottomrule
\end{longtable}

\begin{attention}
Les deux substituts sont volontairement dans \fichier{build-windows/tests/}, à
l'écart de \fichier{WAC.exe}. Placés à côté de lui, ils seraient chargés à la
place des vraies bibliothèques — Windows cherche d'abord dans le dossier du
programme — et \wac{} ne lirait plus aucune ruche, sans erreur visible. Ne
jamais copier le dossier \fichier{tests/} sur un support de collecte.
\end{attention}
...
Lit un \fichier{.evtx} hors de Windows, sous \texttt{wine}. Il sert à éprouver
...
La commande de compilation est dans le README, section \emph{Testing the EVTX
parser outside Windows}.
...
\begin{attention}
Une collecte réelle se traite en local. Si une ruche doit être lue avec le vrai
\fichier{offreg.dll}, elle peut être copiée dans la VM de test, puis effacée
aussitôt le test fini. Aucune copie ne reste dans le dossier de travail
