...
\begin{description}[leftmargin=0pt,itemsep=0.5em]
  ...
  \item[The test programs] (chapter~\ref{ch:harnais}): thirteen executables
    that confront each decoder with a reference (the Microsoft function it
    replaces, a published standard, another implementation), or that prove a
    parser never reads outside its buffer.
...
\begin{itemize}
  \item \texttt{FILE} signature of every \fichier{\$MFT} record and
    \texttt{INDX} signature of every index block, then application of the
    \emph{fixups} (update sequence array), which restore the last bytes of each
    sector;
...
documentation: they check the seal, then the fingerprint of every exhibit.
...
digest of the file found in a verified catalog, or embedded signature. A file
whose authenticity is not established is collected: doubt leads to collecting,
never to discarding.
...
Thirteen executables, left out of \wac{}'s build by their
...
\texttt{system\_conversions\_test} & the Windows functions replaced &
  9,661,284 comparisons, no difference (details below) \\
...
Last cycle, on version 1.3.1 (Windows~11, Paris time zone):

\begin{itemize}
  \item 23 valid JSON files out of 23 produced (the 24th, USBSTOR, is absent:
    the virtual machine has never seen a USB key);
  \item 26 cross-checks passed, no failure, among them 4,045 local dates
    confronted with the IANA database and 145,645 events decoded;
  \item 1,336 exhibits in the exhibit store, exactly those of the manifest,
    seal conforms.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Data the test machine does not hold}: USB keys, MTP devices,
  ...
  \item \textbf{Time zones}: the confrontation with the IANA database covers
    ...
    Windows only.
  \item \textbf{Torn sectors}: the fixups of NTFS records are applied, but
    their sequence number is not yet compared with the end of each sector; a
    torn record is therefore not reported as such.
