...
\begin{lstlisting}
WAC.exe [--collect | --convert=dossier] [--events] [--binary] [--dump]
...
Sans option de mode, \wac{} collecte \textbf{et} convertit en une seule
exécution, sur la machine examinée. Les bonnes pratiques de l'expertise
judiciaire (ISO/IEC~27037 pour la collecte, 27042 pour l'analyse) séparent les
deux :
\begin{itemize}
  \item \textbf{moins d'actions sur la machine examinée}, donc moins de traces :
        la conversion (lecture de milliers de fichiers, mémoire, fichier
        d'échange, rapport d'erreur éventuel) se fait ailleurs ;
  \item \textbf{une conversion refaisable} : à partir de la consigne scellée,
        par un tiers, ou avec une version corrigée de \wac{}, sans revenir sur
        la machine.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Portée de \opt{binary}} : en exécution complète, seuls les
        ...
        La collecte seule dure plus longtemps pour cette raison (16~min~39~s
        contre 4~min~33~s sur la machine de test) : ce temps est de la lecture,
...
\begin{itemize}
  ...
  \item avec \opt{events}, les fichiers de ressources de \textbf{tous} les
        ...
        satellites \fichier{.mui}) : lesquels servent ne se sait qu'à la
        conversion. Environ 650~Mo sur Windows~11 ;
  \item avec \opt{binary}, \textbf{tous} les exécutables, bibliothèques,
        ...
        \begin{itemize}
          \item authentifié (catalogue, signature intégrée, paquet du Store) :
                \textbf{non copié} ; le manifeste garde son chemin, ses empreintes
...
\begin{attention}
Avec \opt{binary}, la collecte lit tout le volume : sur la machine virtuelle
de test (Windows~11 sans logiciel ajouté), 43\,520~exécutables lus, dont
27\,966 authentifiés et 12\,700 fichiers différentiels non copiés,
2\,507 copiés (676~Mo), en 16~minutes ; la consigne entière, journaux
d'événements compris, pèse 1,8~Go. Si la place vient à manquer, les fichiers
...
\begin{itemize}
  \item Un fichier cité qui n'est pas un exécutable (document, donnée) n'est
        pas prélevé : en conversion, il n'a donc \textbf{pas d'empreinte}, là
        où l'exécution complète la calcule sur le disque.
