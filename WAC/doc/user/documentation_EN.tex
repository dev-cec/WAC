...
\begin{lstlisting}
WAC.exe [--collect | --convert=folder] [--events] [--binary] [--dump]
...
Without a mode option, \wac{} collects \textbf{and} converts in a single run,
on the examined machine. Good forensic practice (ISO/IEC~27037 for the
collection, 27042 for the analysis) separates the two:
\begin{itemize}
  \item \textbf{fewer actions on the examined machine}, hence fewer traces: the
        conversion (reading thousands of files, memory, page file, a possible
        error report) happens elsewhere;
  \item \textbf{a conversion that can be redone}: from the sealed exhibit
        store, by a third party, or with a corrected version of \wac{},
...
\begin{itemize}
  ...
  \item \textbf{Scope of \opt{binary}}: in a full run, only the files
        ...
        alone takes longer for that reason (16~min~39~s against 4~min~33~s on the
...
\begin{itemize}
  ...
  \item with \opt{binary}, \textbf{every} executable, library, driver, script
        ...
        \begin{itemize}
          \item authenticated (catalog, embedded signature, Store package):
                \textbf{not copied}; the manifest keeps its path, its
                Authenticode digests and the verdict (\champ{ContentStored}
                \texttt{false});
...
\begin{attention}
With \opt{binary}, the collection reads the whole volume: on the test virtual
machine (Windows~11 without added software), 43,520~executables read, of which
27,966 authenticated and 12,700 differential files not copied, 2,507 copied
(676~MB), in 16~minutes; the whole exhibit store, event logs included, weighs
1.8~GB. If space runs short, the remaining files are hashed without being
...
\begin{itemize}
  \item A cited file that is not an executable (document, data) is not
        collected: at conversion it therefore has \textbf{no fingerprint},
        whereas the full run computes it on the disk.
...
of 2005. For a value the artefact stores in local time (FAT date, Amcache,
BAM…), the hour repeated in autumn is read as its first occurrence, and the
