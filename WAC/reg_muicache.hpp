...
public:
	...
	*/
	HRESULT getData(std::wstring mountpoint, Users userprofiles, bool pdebug) {
		_debug = pdebug;
		HRESULT hresult;
		ORHKEY hKey;
		DWORD nSubkeys;
		...
		for (User userprofile : userprofiles.users) {
			...
			if (hresult != ERROR_SUCCESS) {
				errors.push_back({ L"Unable to open key : " + userprofile.SID + L" / " + replaceAll(userprofile.profile,L"\\",L"\\\\") + L"\\\\AppData\\\\Local\\\\Microsoft\\\\Windows\\\\usrClass.dat / " + L"Local Settings\\\\Software\\\\Microsoft\\\\Windows\\\\Shell\\\\MuiCache", hresult});
				continue;
			};

			hresult = ORQueryInfoKey(hKey, NULL, NULL, &nSubkeys, NULL, NULL, &nValues, NULL, NULL, NULL, NULL);
			if (hresult != ERROR_SUCCESS) {
				errors.push_back({ L"Unable to get info key : " + userprofile.SID + L" / " + replaceAll(userprofile.profile,L"\\",L"\\\\") + L"\\\\AppData\\\\Local\\\\Microsoft\\\\Windows\\\\usrClass.dat / " + L"Local Settings\\\\Software\\\\Microsoft\\\\Windows\\Shell\\\\MuiCache", hresult});
				continue;
			};
			...
			for (int i = 0; i < (int)nValues; i++) {
				...
				if (hresult != ERROR_SUCCESS) {
					errors.push_back({ L"Unable to open key : " + userprofile.SID + L"/ USERCLASS.dat / " + L"Local Settings\\Software\\Microsoft\\Windows\\Shell\\MuiCache\\" + szValue, hresult});
					continue;
				};
				...
				muicache.sid = userprofile.SID;
				muicache.sidName = userprofile.name;
	...
	*/
	HRESULT to_json()
	{
		...
		myfile.open("output/muicache.json");
		myfile << result;
		myfile.close();
		...
		if (_debug == true && errors.size() > 0) {
			...
			myfile.open("errors/muicache_errors.txt");
			myfile << result;
			myfile.close();
		}
